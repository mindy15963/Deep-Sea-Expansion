...
public class PowerUtils
{
	...
	public static String getEnergyDisplay(double energy)
	{
		...
		return getDisplayShort(energy, ElectricUnit.REDSTONE_FLUX);
	}
	...
	public static String getDisplay(double value, ElectricUnit unit, int decimalPlaces, boolean isShort)
	{
		...
		else
		{
			for(int i = 0; i < MeasurementUnit.values().length; i++)
			{
				...
				{
					return prefix + (int)roundDecimals(lowerMeasure.process(value), decimalPlaces) + lowerMeasure.getName(isShort) + unitName;
				}
